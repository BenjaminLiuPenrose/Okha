...
class GenerateStockData(object):
    def __init__(
        ...
        self.image_width = {
            ...
            7: self.bar_width * 7,
            30: self.bar_width * 30,
            90: self.bar_width * 90,
        }
    ...
    def save_annual_data(self):
        ...
        if IS_CRYPTO:
            self.df = cpd.get_processed_BFX_data_by_year(self.year)
        else:
        ...
        for i, stock_id in enumerate(iterator):
            ...
            if IS_CRYPTO:
                dates = cpd.get_period_end_dates(self.freq)
            else:
    ...
    def save_annual_ts_data(self):
        ...
        for i, stock_id in enumerate(iterator):
            # from pdb import set_trace; set_trace()
            df = self.df.xs(stock_id, level=1, drop_level=False).copy()
